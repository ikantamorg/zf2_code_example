...
class Facebook extends \User\Provider\Model\Model
{
    ...
    function initialize()
    {
        if ( ! $this->config["keys"]["id"] || ! $this->config["keys"]["secret"] ){
            throw new \User\Hybrid\Exception( "Your application id and secret are required in order to connect to {$this->providerId}.", 4 );
        }

        if ( isset ( \User\Hybrid\Auth::$config["proxy"] ) ) {
            \User\Third\Facebook\BaseFacebook::$CURL_OPTS[CURLOPT_PROXY] = Hybrid_Auth::$config["proxy"];
        }

        $trustForwarded = isset( $this->config['trustForwarded'] ) ? (bool) $this->config['trustForwarded'] : false;
        $this->api = new \User\Third\Facebook\Facebook( ARRAY( 'appId' => $this->config["keys"]["id"], 'secret' => $this->config["keys"]["secret"], 'trustForwarded' => $trustForwarded ) );
    ...
    function loginBegin()
    {
        ...
        foreach ($optionals as $parameter){
            if( isset( $this->config[$parameter] ) && ! empty( $this->config[$parameter] ) ){
                ...
                if($parameter == "auth_type"){
                    ...
                    \User\Hybrid\Auth::storage()->set('fb_auth_nonce', $nonce);
                }
            }
        }
        ...
        if( isset( $this->config[ 'force' ] ) && $this->config[ 'force' ] === true ){
            ...
            \User\Hybrid\Auth::storage()->set( 'fb_auth_nonce', $parameters[ 'auth_nonce' ] );
        }

        $url = $this->api->getLoginUrl( $parameters );
        \User\Hybrid\Auth::redirect( $url );
    }

    function loginFinish()
    {
        if ( isset( $_REQUEST['error'] ) && $_REQUEST['error'] == "access_denied" ){
            throw new \User\Hybrid\Exception( "Authentication failed! The user denied your request.", 5 );
        }
        ...
        if(isset($_REQUEST['auth_nonce'])){
            $nonce = \User\Hybrid\Auth::storage()->get('fb_auth_nonce');
            \User\Hybrid\Auth::storage()->delete('fb_auth_nonce');
            if($_REQUEST['auth_nonce'] != $nonce){
                throw new \User\Hybrid\Exception( "Authentication failed! Invalid nonce used for reauthentication.", 5 );
            }
        }

        if ( ! $this->api->getUser() ){
            throw new \User\Hybrid\Exception( "Authentication failed! {$this->providerId} returned an invalid user id.", 5 );
        }
