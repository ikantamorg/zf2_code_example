...
class OpenID extends \User\Provider\Model\Model
{
    ...
    function initialize()
    {
        ...
        \User\Hybrid\Auth::$config['proxy'] = isset(\User\Hybrid\Auth::$config['proxy'])?\User\Hybrid\Auth::$config['proxy']:'';

        $hostPort = parse_url( \User\Hybrid\Auth::$config["base_url"], PHP_URL_PORT);
        $hostUrl = parse_url( \User\Hybrid\Auth::$config["base_url"], PHP_URL_HOST);

        if($hostPort) {
            $hostUrl .= ':'.$hostPort;
        }

        $this->api = new \User\Third\OpenID\LightOpenID( $hostUrl, \User\Hybrid\Auth::$config["proxy"] );
    }
    ...
    function loginBegin()
    {
        ...
        \User\Hybrid\Auth::redirect( $this->api->authUrl() );
    }
    ...
    {
        $this->user = \User\Hybrid\Auth::storage()->get( "hauth_session.{$this->providerId}.user" ) ;
